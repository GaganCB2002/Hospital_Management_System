...
export default function SecuritySection() {
  return (
    <section className="relative py-24 lg:py-32 bg-[#0F172A] border-t border-white/[0.04]">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-16">
          {certs.map((cert, i) => (
            <ScrollReveal key={cert.title} delay={i * 0.06}>
              <div className="p-6 rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:bg-white/[0.04] transition-all duration-300 h-full">
                <span className="material-symbols-outlined text-2xl text-[#10B981] mb-3 block">{cert.icon}</span>
                <h3 className="text-base font-bold text-[#F8FAFC] mb-1">{cert.title}</h3>

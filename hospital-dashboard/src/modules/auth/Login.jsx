...
import {
  ...
  FiArrowRight, FiUser, FiActivity, FiMoon, FiSun,
  FiClock as FiTimer
} from 'react-icons/fi';
...
export default function Login() {
  ...
  const { login, loginWithGoogle, signup, rateLimitState, clearRateLimit } = useAuth();
  ...
  const toggleGradient = 'bg-gradient-to-r from-[#0ea5e9] to-[#1e3a8a]';

  return (
    <div className={`min-h-screen w-full relative flex items-center justify-center ${isDark ? 'bg-[#020817]' : 'bg-gradient-to-br from-[#c9d6ff] to-[#e2e2e2]'} transition-colors duration-300`}>
        ...
        >
          ...
          >
            <div className="h-full w-full overflow-y-auto hide-scrollbar py-6">
              <form onSubmit={handleSignUp} className="flex flex-col items-center justify-start px-8 min-h-full">
                ...
                <button type="submit" disabled={isLoading || isRateLimited}
                  ...
                  {isLoading ? 'Creating Account...' : isRateLimited ? `Wait ${countdown}s` : 'Sign Up'}
                </button>
          ...
          >
            <form onSubmit={handleSignIn} className="flex items-center justify-center flex-col px-10 h-full w-full">
              ...
              <div className="flex items-center justify-between w-full mb-3.5">
                ...
                <button type="button" onClick={handleForgotPasswordClick} className={`text-[10px] font-bold bg-transparent border-none cursor-pointer ${isDark ? 'text-sky-400' : 'text-[#512da8] hover:underline'}`}>
                  Forgot Password?
                </button>
              </div>
                ...
                <div className="flex items-center gap-2 mb-4">
                  <div className="w-9 h-9 rounded-xl bg-white/10 flex items-center justify-center text-white backdrop-blur-sm">
                    <svg className="w-5.5 h-5.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12h1.5l2.25-4.5L11.25 18l2.25-10.5 2.25 7.5h1.5l1.5-3" />
                    </svg>
                  </div>
